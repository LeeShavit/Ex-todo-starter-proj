...
function query() {
    return Promise.resolve(users.map(user => ({ _id: user._id, fullname: user.fullname, balance: user.balance, prefs: user.prefs, activities: user.activities })))
}
...
function save(userToSave) {
    ...
    return _saveUsersToFile()
        .then(() => ({
            ...
            prefs: userToSave.prefs,
            activities: userToSave.activities
        }))
}
